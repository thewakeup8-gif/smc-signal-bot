...
def format_signal(signal):
    ...
    ip = f"`{signal.institution_price:.4f}`" if signal.institution_price else "Unknown"
    return f"""🏛️ *INSTITUTIONAL SIGNAL*
📊 *{signal.symbol}* | `{signal.timeframe}` | {emoji}
━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 Entry:     `{signal.entry}`
🛑 Stop Loss: `{signal.sl}`
🏆 TP1 (2R):  `{signal.tp1}`
...
📐 R:R: `1:{signal.rr}`
━━━━━━━━━━━━━━━━━━━━━━━━━━
📌 Phase: {signal.phase} | Trend: {signal.trend_htf}
🏛️ {signal.institution_verdict}
💵 Their Price: {ip}
{signal.bulk_verdict}
📊 Confidence: `{signal.confidence:.1f}%` [{bar}]
👥 Pro Agreement: `{signal.pro_agreement}/100`
━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️ Max 1% risk | 🕐 {datetime.utcnow().strftime('%H:%M UTC')}""".strip()
...
async def main():
    ...
    if signals_found:
        for sig in signals_found:
            await bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=format_signal(sig), parse_mode="Markdown")
            await asyncio.sleep(1)
    else:
        await bot.send_message(
            chat_id=TELEGRAM_CHAT_ID,
            text=f"🔍 *Scan Complete* — {datetime.utcnow().strftime('%H:%M UTC')}\nNo setup ≥{MIN_CONFIDENCE}% found.\nCapital protected. 🛡️",
            parse_mode="Markdown"
        )
